...
public class JTableAPP extends JFrame{ //외부 클래스
	...
	//class ShowTable extends JPanel implements MouseListener{ // MouseListener 추상메소드 5개중 1개만 필요 MouseAdapter
		...
		//필드 선언
		//마우스 리스너를 전체 선언하면 5개의 오버라이드가 생긴다 그중에서 마우스 어뎁터는 필요한 기능만 있다!!!	 implements MouseListener
			//어뎁터가 존재하는 경우는 추상메소드가 하나 이상일경우!!!!
			//포커스 -> Actionlistner -> 1개(어뎁터 존재x)  Itemlistener-> 1개 (어뎁터 존재x)  MouseListner->5개 필요한거만 재정의  마우스어뎁터->(클래스) 
			
			
			
			//필요한 추상메소드 수동으로 오버라이드 하는 방법 -> 어뎁터는 자동 오버라이드 안됨!! -> 수동오버라이드 방법 -> m ctrl spacebar (m으로 시작하는 오버라이드 함수 )
		
		DefaultTableModel datamodel;  //테이블에 추가삭제 수정용
		JTable table;
